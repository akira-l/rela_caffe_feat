...
import os
from need.imdb import imdb
import need.ds_utils as ds_utils
...
class vg(imdb):
    def __init__(self, version, image_set, xml_path):
    ...
    def _annotation_path(self, index):
        return os.path.join(self._data_path, self.xml_path, str(index) + '.xml')
    ...
    def _do_python_eval(self, output_dir, pickle_=True, eval_attributes = False):
        ...
        #pdb.set_trace()
        tmp_cls_reco = np.zeros(len(classes))
        for tmp_gt in gt_roidb:
            tmp_classes = tmp_gt['gt_classes']
            for tmp_cls in tmp_classes:
                tmp_cls_reco[tmp_cls] += 1
        met_cls = np.nonzero(tmp_cls_reco)[0].size
        ...
        print('met classes Mean AP = {:.4f}'.format(sum(aps)/met_cls))
        pdb.set_trace()
